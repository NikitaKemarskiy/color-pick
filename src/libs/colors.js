...
async function getColors(imgPath) {
	...
	for (let i = 0; i < bytes.length; i += 4) {
		...
		colors.push({
			// Push color into the colors array
			red,
			green,
			blue
		});
...
function pickColors(colors, factor) {
	if (colors.length === 1) {
		return colors;
	} // Colors has one item -> return it
	if (colors.length === 0) {
		return null;
	} // Colors has no items -> return null
	if (factor === 1) {
		return [getAverageColor(colors)];
	} // Factor is 1 -> return average color

	const red = { min: 255, max: 0 };
	const green = { min: 255, max: 0 };
	const blue = { min: 255, max: 0 };
	colors.forEach((color) => {
		if (color.red < red.min) {
			red.min = color.red;
		}
		if (color.red > red.max) {
			red.max = color.red;
		}
		if (color.green < green.min) {
			green.min = color.green;
		}
		if (color.green > green.max) {
			green.max = color.green;
		}
		if (color.blue < blue.min) {
			blue.min = color.blue;
		}
		if (color.blue > blue.max) {
			blue.max = color.blue;
		}
	});
	...
	const arrTwo = pickColors(
		colors.slice(colors.length / 2),
		floor(factor / 2)
	);
